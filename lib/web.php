...
class Web extends Prefab
{
    ...
    protected function _socket($url, $options)
    {
        ...
        if ($socket = @fsockopen($parts['host'], $parts['port'], $code, $err)) {
            ...
            if (
                $options['follow_location'] &&
                preg_grep('/HTTP\/1\.\d 3\d{2}/', $headers) &&
                preg_match('/Location: (.+?)' . preg_quote($eol) . '/', $html[0], $loc)
            ) {
                $options['max_redirects']--;
                return $this->request($loc[1], $options);
            }
    ...
    public function request($url, array $options = null)
    {
        ...
        if (isset($parts['user'], $parts['pass'])) {
            $this->subst(
                $options['header'],
                'Authorization: Basic ' . base64_encode($parts['user'] . ':' . $parts['pass'])
            );
        }
        $options += [
            'method'          => 'GET',
        ...
        if (
            ...
            if (
                $cache->exists($hash = $fw->hash($options['method'] . ' ' . $url) . '.url', $data)
            ) {
                if (
                    preg_match(
        ...
        if ($result && isset($cache)) {
            if (
                ...
                preg_match(
                    '/Cache-Control:(?:.*)max-age=(\d+)(?:,?.*' . preg_quote($eol) . ')/',
                    implode($eol, $result['headers']),
                    $exp
                )
            ) {
